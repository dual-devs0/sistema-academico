...
| # | Hallazgo | Archivos | Acción |
|---|----------|----------|--------|
| 1 | **Double-query en `resolver_materia`** | `services/equivalencia.py:86-91` | Refactor: consultar `Materia` una sola vez y cachear resultado |
...
| # | Hallazgo | Archivos | Acción |
|---|----------|----------|--------|
| 1 | **Direct `fetch()` en `Boleta.tsx`** | `pages/Boleta.tsx:114` | Migrar a `api.get()` |
| 2 | **Pages ignoran services** | Todos los pages | Opcional: migrar pages a usar los services existentes (`agendaService`, `pensumService`, etc.) |
| 3 | **Hardcoded URLs** | `finanzasService.ts`, `pasantiasService.ts`, `tramitesService.ts` | Eliminar hardcoded `http://localhost:8000` — el proxy de Vite ya redirige |
...
- [ ] `alembic current` muestra 1 solo head
- [ ] Todos los tests de backend pasan: `pytest backend/tests/ -v`
- [ ] `npm run build` (frontend) sin errores
- [ ] `npm run typecheck` (frontend) sin errores
- [ ] Probar login manual
- [ ] Probar descarga de rendición (becas)
- [ ] Probar aprobar pasantía
- [ ] Probar resolver trámite
- [ ] Verificar que `GET /carreras/` requiera auth
