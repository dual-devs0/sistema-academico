...
- **Reset de contraseña** *(2026-08-12)*: `POST /auth/recuperar-contrasena` crea un token `secrets.token_urlsafe(32)` guardado SOLO hasheado (SHA-256) en `password_reset_tokens` con expiración `RESET_TOKEN_EXPIRATION_MINUTES` (60), invalida links previos, y envía email REAL con link (`RESET_PASSWORD_FRONTEND_URL`) vía proveedor intercambiable (Resend/SendGrid/SMTP/mock, nunca falla el request). `POST /auth/reset-password` valida hash+expiración, consume el token, borra pendientes y revoca TODOS los refresh tokens del usuario. Respuesta siempre genérica 200 (sin enumeración). Auditoría en `audit_log`.
- **Rate limiting global** *(2026-08-12)*: middleware ASGI en `app/rate_limiting.py` (reemplaza slowapi). Sliding window log por request: buckets por IP (X-Forwarded-For), usuario JWT y API key (`X-API-Key` en `API_KEYS`) — se exigen TODOS. Storage Redis con degradación a memoria local si cae. Categorías (AUTH_LOGIN, AUTH_REGISTER, PASSWORD_RESET, UPLOADS, QR, REPORTS, SEARCH, HEAVY_ENDPOINTS, ADMIN, EXPORTS, IMPORTS, READ/WRITE genéricos) con límites por env `RATE_LIMIT_<CATEGORIA>=N/minutes`. 429 JSON consistente + headers `X-RateLimit-Limit`/`X-RateLimit-Remaining`/`Retry-After`. Excluye `/health,/version,/metrics,/` y `/static,/docs,/openapi.json,/redoc`. Métricas en `GET /metrics` (Prometheus).
