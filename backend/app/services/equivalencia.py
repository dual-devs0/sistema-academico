...
from app.models.equivalencia import SolicitudEquivalencia, EquivalenciaMateria, ExamenSuficiencia
...
def crear_solicitud(alumno_id: int, tipo: str, universidad_origen: Optional[str],
                    db: Session) -> SolicitudEquivalencia:
    pendiente = db.query(SolicitudEquivalencia).filter(
...
def resolver_materia(solicitud_id: int, materia_eq_id: int,
                     resolucion: str, materia_destino_id: Optional[int],
                     db: Session) -> EquivalenciaMateria:
    eq = db.query(EquivalenciaMateria).filter(EquivalenciaMateria.id == materia_eq_id,
                                               EquivalenciaMateria.solicitud_id == solicitud_id).first()
    if not eq:
    ...
    if resolucion == "aprobada" and materia_destino_id is not None:
        solicitud = db.query(SolicitudEquivalencia).filter(
            SolicitudEquivalencia.id == solicitud_id).first()
        if solicitud:
            alumno_id = solicitud.alumno_id

            pensum = db.query(PensumMateria).filter(
                PensumMateria.materia_id == materia_destino_id,
                PensumMateria.carrera_id == (
                    db.query(Materia).filter(Materia.id == materia_destino_id).first().carrera_id
                ),
            ).first()
            creditos = pensum.creditos if pensum else 4

            # Crear oferta sintética para expediente si no existe
            oferta = db.query(OfertaMateria).filter(
            ...
            existente = db.query(ExpedienteMateria).filter(
            ...
            if not existente:
                exp = ExpedienteMateria(
                    ...
                    nota_final=6.0, creditos=creditos,
                    condicion="aprobada", cerrado_por=alumno_id,
                )
                db.add(exp)

            avance = db.query(AvanceAlumnoPensum).filter(
                AvanceAlumnoPensum.alumno_id == alumno_id,
                AvanceAlumnoPensum.pensum_materia_id == pensum.id,
            ).first() if pensum else None
            if avance:
                avance.estado = "aprobada"
            elif pensum:
                avance_nuevo = AvanceAlumnoPensum(
                    alumno_id=alumno_id, pensum_materia_id=pensum.id, estado="aprobada",
                )
                db.add(avance_nuevo)
...
def crear_examen_suficiencia(alumno_id: int, materia_id: int, fecha: date,
                             db: Session) -> ExamenSuficiencia:
...
def obtener_equivalencias_alumno(alumno_id: int, db: Session) -> list[SolicitudEquivalencia]:
    return db.query(SolicitudEquivalencia).filter(
        SolicitudEquivalencia.alumno_id == alumno_id,
    ).all()