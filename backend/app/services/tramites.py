...
def _footer(story: list) -> None:
    ...
    story.append(
        Paragraph(
            f"Fecha de emisión: {fecha_txt} · Documento generado automáticamente por el Sistema Académico UCA",  # noqa: E501
            ParagraphStyle(
                "disc",
                parent=styles["Normal"],
                fontSize=7,
...
def generar_constancia_regular_pdf(alumno: User, regularidad: dict) -> bytes:
    ...
    if regularidad.get("ppa_acumulado") is not None:
        ...
        story.append(
            Paragraph(f"PPA acumulado: {regularidad['ppa_acumulado']}", style_body)
        )
    _footer(story)
...
def generar_historial_oficial_pdf(alumno: User, db: Session) -> bytes:
        ...
        table_data.append(
            [
                Paragraph(
                    periodo,
                    ParagraphStyle(
                        "p", parent=styles["Normal"], fontSize=9, alignment=TA_CENTER
                    ),
                ),
                Paragraph(
                    nombre, ParagraphStyle("m", parent=styles["Normal"], fontSize=9)
                ),
                Paragraph(
                    str(exp.nota_final),
                    ParagraphStyle(
                        "n", parent=styles["Normal"], fontSize=9, alignment=TA_CENTER
                    ),
                ),
                Paragraph(
                    exp.condicion,
                    ParagraphStyle(
                        "c", parent=styles["Normal"], fontSize=9, alignment=TA_CENTER
                    ),
                ),
            ]
        )
    if not rows:
        table_data.append(
            [
                Paragraph(
                    "Sin materias registradas en el expediente",
                    ParagraphStyle(
                        "e",
                        parent=styles["Normal"],
                        fontSize=9,
...
_GENERADORES_AUTO: dict[str, Callable[[User, Session], bytes]] = {
    "Constancia de alumno regular": lambda alumno, db: generar_constancia_regular_pdf(
        alumno, calcular_regularidad(alumno.id, db)
    ),
...
def crear_solicitud(alumno_id: int, tipo_tramite_id: int, db: Session) -> Solicitud:
    ...
    if not tipo.requiere_aprobacion and generador:
        ...
        if regularidad["estado"] != "activo":
            raise ValueError(
                f"No se puede generar '{tipo.nombre}': "
                f"el alumno no está en estado regular "
