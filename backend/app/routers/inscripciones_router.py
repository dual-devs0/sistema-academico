...
def inscribir(inscripcion: schemas.inscripcion.InscripcionCreate, db: Session = Depends(database.SessionLocal), current_user = Depends(get_current_user)):
...
def listar_inscripciones(alumno_id: int, db: Session = Depends(database.SessionLocal), current_user = Depends(get_current_user)):
