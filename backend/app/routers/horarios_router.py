...
def verificar_solapamiento_inscripcion(db: Session, alumno_id: int, materia_id_nueva: int) -> list[str]:
    """
    Verifica si el horario de la materia nueva se superpone con materias ya cursadas por el alumno.
    Retorna lista de descripciones de conflictos.
    """
    conflictos = []
    horario_nuevo = db.query(models.horario.Horario).filter(
        models.horario.Horario.materia_id == materia_id_nueva
    ).all()
    ...
    for h_nuevo in horario_nuevo:
        ...
        for h_exist in horarios_exist:
            if h_nuevo.hora_inicio < h_exist.hora_fin and h_nuevo.hora_fin > h_exist.hora_inicio:
                materia_exist = db.query(models.materia.Materia).filter(
                    models.materia.Materia.id == h_exist.materia_id
                ).first()
                conflictos.append(
                    f"'{materia_exist.nombre if materia_exist else '?'}' el día {_dia_nombre(h_exist.dia_semana)} "
...
def create_horario(
    ...
    if current_user["role"] not in ("admin", "profesor"):
        raise HTTPException(status_code=403, detail="Solo administradores y profesores pueden crear horarios")
    ...
    existing = db.query(models.horario.Horario).filter(
    ...
    if existing:
        raise HTTPException(status_code=400, detail="Ya existe un horario para esta materia en ese día y hora")
    new_horario = models.horario.Horario(**horario.model_dump())
    db.add(new_horario)
    db.commit()
    db.refresh(new_horario)
    materia_nombre = db.query(models.materia.Materia.nombre).filter(
        models.materia.Materia.id == new_horario.materia_id
    ).scalar()
...
def list_horarios(
    ...
    horarios = query.order_by(models.horario.Horario.dia_semana, models.horario.Horario.hora_inicio).all()
    result = []
    for h in horarios:
        materia_nombre = db.query(models.materia.Materia.nombre).filter(
            models.materia.Materia.id == h.materia_id
        ).scalar()
        result.append(schemas.horario.HorarioOut(
            id=h.id, materia_id=h.materia_id, dia_semana=h.dia_semana,
...
def horarios_por_materia(
    ...
    horarios = db.query(models.horario.Horario).filter(
        models.horario.Horario.materia_id == materia_id
    ).order_by(models.horario.Horario.dia_semana, models.horario.Horario.hora_inicio).all()
    materia_nombre = db.query(models.materia.Materia.nombre).filter(
...
def delete_horario(
    ...
    horario = db.query(models.horario.Horario).filter(models.horario.Horario.id == horario_id).first()
    if not horario:
...
def verificar_solapamiento(
    ...
    """Verifica si el alumno actual tiene solapamiento de horario al inscribirse en una materia."""
    if current_user["role"] != "alumno":
        raise HTTPException(status_code=403, detail="Solo alumnos pueden verificar solapamiento")
    conflictos = verificar_solapamiento_inscripcion(db, current_user["user_id"], materia_id)
    return {
