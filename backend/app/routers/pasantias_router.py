...
from app.schemas.pasantia import (
    EmpresaReceptoraCreate, EmpresaReceptoraOut,
    PasantiaCreate, PasantiaOut, PasantiaHorasUpdate,
    InformePasantiaCreate, InformePasantiaOut,
)
from app.services.pasantia import (
    crear_empresa, crear_solicitud_pasantia, aprobar_pasantia,
    actualizar_horas, subir_informe, finalizar_pasantia,
)
...
@router.post("/empresas", response_model=EmpresaReceptoraOut, summary="Admin registra empresa receptora")
def crear_empresa_endpoint(
    ...
    try:
        empresa = crear_empresa(
            nombre=data.nombre, rubro=data.rubro, contacto=data.contacto,
            telefono=data.telefono, email=data.email,
            convenio_activo=data.convenio_activo, db=db,
        )
        db.commit()
        db.refresh(empresa)
        return empresa
...
@router.post("/solicitudes", response_model=PasantiaOut, summary="Alumno solicita pasantía")
def crear_solicitud_endpoint(
    ...
    try:
        pasantia = crear_solicitud_pasantia(
            alumno_id=current_user["user_id"], empresa_id=data.empresa_id,
            fecha_inicio=data.fecha_inicio, horas_requeridas=data.horas_requeridas, db=db,
        )
        db.commit()
        db.refresh(pasantia)
        return pasantia
...
@router.put("/{id}/aprobar", response_model=PasantiaOut, summary="Admin/profesor aprueba pasantía")
...
@router.put("/{id}/horas", response_model=PasantiaOut, summary="Actualiza horas completadas")
...
@router.post("/{id}/informes", response_model=InformePasantiaOut, summary="Subir informe de pasantía")
