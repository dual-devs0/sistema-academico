...
def listar_hilos(
    ...
    hilos = query.order_by(
        models.foro.ForoHilo.fijado.desc(), models.foro.ForoHilo.created_at.desc()
    ).all()
    ...
    for h in hilos:
        creador = (
            ...
            .filter(models.user.User.id == h.creado_por)
            .first()
        )
        result.append(
            schemas.foro.ForoHiloOut(
                id=h.id,
...
def obtener_hilo(
    ...
    hilo = (
        ...
        .filter(models.foro.ForoHilo.id == hilo_id)
        .first()
    )
    if not hilo:
    ...
    creador = (
        db.query(models.user.User)
        .filter(models.user.User.id == hilo.creado_por)
        .first()
    )

    # Los mensajes ya no viajan embebidos -- ver GET /foro/hilos/{hilo_id}/mensajes (paginado)  # noqa: E501
...
@router.get(
    "/hilos/{hilo_id}/mensajes", response_model=schemas.foro.ForoMensajesListOut
)
def listar_mensajes(
    hilo_id: int,
    skip: int = 0,
    ...
    query = db.query(models.foro.ForoMensaje).filter(
        models.foro.ForoMensaje.hilo_id == hilo_id
    )
    total = query.count()
    # Más recientes primero para "cargar más" hacia atrás en el tiempo
    mensajes_q = (
        query.order_by(models.foro.ForoMensaje.created_at.desc())
        .offset(skip)
        .limit(limit)
        .all()
    )
    ...
    for m in mensajes_q:
        autor = (
            db.query(models.user.User).filter(models.user.User.id == m.user_id).first()
        )
        items.append(
            schemas.foro.ForoMensajeOut(
                id=m.id,
                hilo_id=m.hilo_id,
                user_id=m.user_id,
...
def crear_mensaje(
    ...
    autor = (
        db.query(models.user.User).filter(models.user.User.id == nuevo.user_id).first()
    )
    return schemas.foro.ForoMensajeOut(
        id=nuevo.id,
        hilo_id=nuevo.hilo_id,
...
def editar_mensaje(
    ...
    msg = (
        db.query(models.foro.ForoMensaje)
        .filter(models.foro.ForoMensaje.id == mensaje_id)
        .first()
    )
    if not msg:
    ...
    creado = (
        ...
        else msg.created_at.replace(tzinfo=timezone.utc)
    )
    if ahora - creado > VENTANA_EDICION_MENSAJE:
        raise HTTPException(
            status_code=403, detail="La ventana de edición de 15 minutos ya venció"
        )
    ...
    autor = (
        db.query(models.user.User).filter(models.user.User.id == msg.user_id).first()
    )
    return schemas.foro.ForoMensajeOut(
        id=msg.id,
        hilo_id=msg.hilo_id,
