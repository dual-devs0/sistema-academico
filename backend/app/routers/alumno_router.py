...
def mis_notas(
    ...
    for p in puntajes:
        ...
        if mid not in por_materia:
            por_materia[mid] = {"materia_id": mid, "materia_nombre": mat_map.get(mid, "—"), "parcial1": None, "parcial2": None, "practico": None, "final": None}
    ...
    for mid, data in por_materia.items():
        ...
        if existentes:
            ...
            prom = round(sum(PESOS[k] * v for k, v in existentes.items()) / peso_total, 2) if peso_total > 0 else None
        else:
            prom = None
...
def mi_asistencia(
    ...
    user_id = current_user["user_id"]
    asistencias = db.query(models.asistencia.Asistencia).filter(models.asistencia.Asistencia.user_id == user_id).all()
...
def mi_resumen(
    ...
    return {
        "alumno": {"id": user.id, "nombre": user.nombre, "username": user.username, "email": user.email, "es_becado": user.es_becado} if user else None,
