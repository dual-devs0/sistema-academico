...
from app.schemas.graduacion import (
    CondicionEgresoOut, ProcesoGraduacionCreate, ProcesoGraduacionOut,
    ProcesoGraduacionTutorUpdate, EtapaTesisCreate, EtapaTesisUpdate,
    EtapaTesisOut, VerificacionSolvenciaOut,
)
from app.services.graduacion import (
    verificar_condicion_egreso, iniciar_proceso, asignar_tutor,
    actualizar_etapa,
)
...
@router.get("/alumno/{id}/condicion", response_model=CondicionEgresoOut,
...
@router.post("/procesos", response_model=ProcesoGraduacionOut,
...
@router.put("/procesos/{id}/tutor", response_model=ProcesoGraduacionOut,
...
@router.put("/procesos/{id}/etapas/{eid}", response_model=EtapaTesisOut,
...
def actualizar_etapa_endpoint(
    ...
    try:
        etapa = actualizar_etapa(etapa_id=eid, estado=data.estado,
                                  observaciones=data.observaciones, db=db)
        db.commit()
        db.refresh(etapa)
        return etapa
    except ValueError as e:
...
@router.get("/procesos/{id}/solvencia", response_model=List[VerificacionSolvenciaOut],
...
def listar_solvencia(
    ...
    return db.query(VerificacionSolvencia).filter(
        VerificacionSolvencia.proceso_id == id,
    ).all()


@router.get("/procesos/{id}/documentos-cones", summary="Listar URL de documentos de Cones")
