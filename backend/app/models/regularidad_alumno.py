...
class RegularidadAlumno(Base):
    ...
    calculado_en = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())

    __table_args__ = (
        CheckConstraint("estado IN ('activo','en_riesgo','irregular','de_baja')", name="ck_regularidad_estado"),
    )
