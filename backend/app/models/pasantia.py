...
class EmpresaReceptora(Base):
    ...
    pasantias = relationship("Pasantia", back_populates="empresa", cascade="all, delete-orphan")
...
class Pasantia(Base):
    ...
    empresa_id: Mapped[int] = mapped_column(Integer, ForeignKey("empresas_receptoras.id", ondelete="CASCADE"), nullable=False)
    tutor_academico_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    ...
    informes = relationship("InformePasantia", back_populates="pasantia", cascade="all, delete-orphan")
...
class InformePasantia(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    pasantia_id: Mapped[int] = mapped_column(Integer, ForeignKey("pasantias.id", ondelete="CASCADE"), nullable=False)
