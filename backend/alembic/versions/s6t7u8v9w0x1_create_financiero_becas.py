...
def upgrade() -> None:
    # Idempotente: algunos entornos (p.ej. neondb_test) ya tenían estas
    # tablas creadas por el extinto create_all() previo a la migración a
    # Alembic-only. Se guarda cada paso con inspector.has_table/has_column
    # para poder correr esta migración de forma segura sea cual sea el
    ...
    # ── fuentes_beca ─────────────────────────────────────────────────────
    if not insp.has_table("fuentes_beca"):
        op.create_table(
            "fuentes_beca",
    ...
    ya_sembrado = bind.execute(sa.text("SELECT COUNT(*) FROM fuentes_beca")).scalar()
    if not ya_sembrado:
        op.bulk_insert(
            fuentes_table,
            [
    ...
    # ── becas_catalogo ───────────────────────────────────────────────────
    if not insp.has_table("becas_catalogo"):
        op.create_table(
            "becas_catalogo",
            sa.Column("id", sa.Integer(), primary_key=True),
    ...
    # ── postulaciones_beca ───────────────────────────────────────────────
    if not insp.has_table("postulaciones_beca"):
        op.create_table(
            "postulaciones_beca",
            sa.Column("id", sa.Integer(), primary_key=True),
    ...
    # ── becas_activas ────────────────────────────────────────────────────
    if not insp.has_table("becas_activas"):
        op.create_table(
            "becas_activas",
            sa.Column("id", sa.Integer(), primary_key=True),
            sa.Column("alumno_id", sa.Integer(), sa.ForeignKey("users.id"), nullable=False),
    ...
    # ── conceptos_arancel ────────────────────────────────────────────────
    if not insp.has_table("conceptos_arancel"):
        op.create_table(
            "conceptos_arancel",
            sa.Column("id", sa.Integer(), primary_key=True),
            sa.Column("nombre", sa.String(200), nullable=False),
    ...
    # ── comprobantes ─────────────────────────────────────────────────────
    if not insp.has_table("comprobantes"):
        op.create_table(
            "comprobantes",
            sa.Column("id", sa.Integer(), primary_key=True),
    ...
    # ── auditoria_override_mora ───────────────────────────────────────────
    if not insp.has_table("auditoria_override_mora"):
        op.create_table(
            "auditoria_override_mora",
            sa.Column("id", sa.Integer(), primary_key=True),
            sa.Column("alumno_id", sa.Integer(), sa.ForeignKey("users.id"), nullable=False),
