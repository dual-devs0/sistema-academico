...
def upgrade() -> None:
    ...
    if not insp.has_table("empresas_receptoras"):
        op.create_table(
            ...
            sa.Column(
                "convenio_activo", sa.Boolean(), nullable=False, server_default="false"
            ),
        )
    ...
    if not insp.has_table("pasantias"):
        op.create_table(
            ...
            sa.Column(
                "empresa_id",
                sa.Integer(),
                sa.ForeignKey("empresas_receptoras.id"),
                nullable=False,
            ),
            sa.Column(
                "tutor_academico_id",
            ...
            sa.Column(
                "horas_completadas", sa.Integer(), nullable=False, server_default="0"
            ),
            sa.Column(
    ...
    if not insp.has_table("procesos_graduacion"):
        op.create_table(
            ...
            sa.Column(
                "fecha_inicio", sa.DateTime(timezone=True), server_default=sa.func.now()
            ),
            sa.Column(
                "estado", sa.String(20), nullable=False, server_default="en_proceso"
            ),
            sa.Column(
                "tutor_id", sa.Integer(), sa.ForeignKey("users.id"), nullable=True
            ),
            sa.CheckConstraint(
                "estado IN ('en_proceso','tesis_en_curso','tesis_aprobada','graduado','rechazado')",  # noqa: E501
    ...
    if not insp.has_table("etapas_tesis"):
        op.create_table(
            ...
            sa.Column(
                "proceso_id",
                sa.Integer(),
                sa.ForeignKey("procesos_graduacion.id"),
                nullable=False,
            ),
    ...
    if not insp.has_table("equivalencias_materia"):
        op.create_table(
            ...
            sa.Column(
                "solicitud_id",
                sa.Integer(),
                sa.ForeignKey("solicitudes_equivalencia.id"),
                nullable=False,
            ),
