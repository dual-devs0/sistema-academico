...
def upgrade() -> None:
    op.add_column("carreras", sa.Column("duracion_semestres", sa.Integer(), nullable=True))
    op.add_column("carreras", sa.Column("creditos_totales", sa.Integer(), nullable=True))

    op.create_table(
        "pensum_materias",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("carrera_id", sa.Integer(), sa.ForeignKey("carreras.id"), nullable=False),
        ...
        sa.UniqueConstraint("carrera_id", "materia_id", name="uq_pensum_carrera_materia"),
    )
    ...
    op.create_table(
        ...
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("materia_id", sa.Integer(), sa.ForeignKey("materias.id"), nullable=False),
        sa.Column("prerrequisito_id", sa.Integer(), sa.ForeignKey("materias.id"), nullable=False),
        sa.Column("tipo", sa.String(20), nullable=False),
        sa.CheckConstraint("materia_id != prerrequisito_id", name="ck_correlatividad_no_autorreferencia"),
        sa.UniqueConstraint("materia_id", "prerrequisito_id", "tipo", name="uq_correlatividad_regla"),
    )
    ...
    op.create_table(
        ...
        sa.Column("pensum_materia_id", sa.Integer(), sa.ForeignKey("pensum_materias.id"), nullable=False),
        sa.Column("estado", sa.String(20), nullable=False, server_default="pendiente"),
        sa.Column("fecha_actualizacion", sa.DateTime(timezone=True), server_default=sa.func.now()),
        sa.UniqueConstraint("alumno_id", "pensum_materia_id", name="uq_avance_alumno_pensum_materia"),
    )
