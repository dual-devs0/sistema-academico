...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("oferta_materia_id", sa.Integer(), sa.ForeignKey("ofertas_materia.id"), nullable=False),
        ...
        sa.Column("cerrado_por", sa.Integer(), sa.ForeignKey("users.id"), nullable=False),
        sa.Column("cerrado_en", sa.DateTime(timezone=True), server_default=sa.func.now()),
        sa.CheckConstraint("condicion IN ('aprobada','reprobada')", name="ck_expediente_condicion"),
        sa.UniqueConstraint("alumno_id", "oferta_materia_id", name="uq_expediente_alumno_oferta"),
    )
    ...
    op.create_table(
        ...
        sa.Column("materias_aprobadas", sa.Integer(), nullable=False, server_default="0"),
        sa.Column("materias_reprobadas", sa.Integer(), nullable=False, server_default="0"),
        sa.UniqueConstraint("alumno_id", "periodo", name="uq_expediente_semestre_alumno_periodo"),
    )

    op.create_table(
        "regularidad_alumno",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("alumno_id", sa.Integer(), sa.ForeignKey("users.id"), nullable=False, unique=True),
        ...
        sa.CheckConstraint("estado IN ('activo','en_riesgo','irregular','de_baja')", name="ck_regularidad_estado"),
    )
