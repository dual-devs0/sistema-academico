...
carrera = next((c for c in db.query(Carrera).all() if normalizar(c.nombre) == target_carrera), None)
if not carrera:
    carrera = Carrera(nombre="Ing. Informatica", duracion_semestres=10, creditos_totales=240)
    db.add(carrera)
    db.flush()
...
for nombre in materias_data:
    ...
    if not m:
        m = Materia(nombre=nombre, carrera_id=carrera.id, anio=1, semestre=1, creditos=4)
        db.add(m)
        db.flush()
...
for m in (analisis, fisica, programacion):
    pm = db.query(PensumMateria).filter(
        PensumMateria.carrera_id == carrera.id,
        PensumMateria.materia_id == m.id,
    ).first()
    if not pm:
        pm = PensumMateria(carrera_id=carrera.id, materia_id=m.id, semestre=1, creditos=4, es_electiva=False)
        db.add(pm)
        db.flush()
        print(f"  + PensumMateria creada: {m.nombre} -> semestre 1, creditos=4 (id={pm.id})")
    else:
        print(f"  = PensumMateria ya existe: {m.nombre} (id={pm.id})")

# 4. correlatividad: Fisica I exige aprobada Analisis Matematico I
corr = db.query(Correlatividad).filter(
    Correlatividad.materia_id == fisica.id,
    Correlatividad.prerrequisito_id == analisis.id,
    Correlatividad.tipo == "aprobada",
).first()
if not corr:
    corr = Correlatividad(materia_id=fisica.id, prerrequisito_id=analisis.id, tipo="aprobada")
    db.add(corr)
    db.flush()
    print(f"  + Correlatividad creada: Fisica I (id={fisica.id}) exige aprobada Analisis Matematico I (id={analisis.id})")
else:
    print(f"  = Correlatividad ya existe: Fisica I exige aprobada Analisis Matematico I")
...
print(f"carrera_id={carrera.id}  analisis_id={analisis.id}  fisica_id={fisica.id}  programacion_id={programacion.id}  maria_id={maria.id}")
