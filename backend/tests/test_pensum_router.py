...
def test_agregar_materia_a_malla_admin(client, seed, tokens, db):
    ...
    res = client.post(f"/pensum/carreras/{seed['carrera'].id}/materias", json=payload, headers=auth(tokens["admin"]))
...
def test_agregar_materia_a_malla_no_admin_403(client, seed, tokens, db):
    ...
    payload = {"materia_id": materia.id, "semestre": 1, "creditos": 6}
    res = client.post(f"/pensum/carreras/{seed['carrera'].id}/materias", json=payload, headers=auth(tokens["profesor"]))
...
def test_agregar_materia_duplicada_400(client, seed, tokens, db):
    ...
    payload = {"materia_id": materia.id, "semestre": 1, "creditos": 6}
    r1 = client.post(f"/pensum/carreras/{seed['carrera'].id}/materias", json=payload, headers=auth(tokens["admin"]))
    assert r1.status_code == 200
    r2 = client.post(f"/pensum/carreras/{seed['carrera'].id}/materias", json=payload, headers=auth(tokens["admin"]))
...
def test_quitar_materia_de_malla(client, seed, tokens, db):
    ...
    res = client.post(f"/pensum/carreras/{seed['carrera'].id}/materias", json=payload, headers=auth(tokens["admin"]))
    pm_id = res.json()["id"]
    res = client.delete(f"/pensum/carreras/{seed['carrera'].id}/materias/{pm_id}", headers=auth(tokens["admin"]))
    assert res.status_code == 200
...
def test_crear_correlatividad_autorreferencia_422(client, seed, tokens, db):
    ...
    res = client.post("/pensum/correlatividades", json={
        "materia_id": materia.id, "prerrequisito_id": materia.id, "tipo": "aprobada",
...
def test_crear_y_eliminar_correlatividad(client, seed, tokens, db):
    ...
    res = client.delete(f"/pensum/correlatividades/{corr_id}", headers=auth(tokens["admin"]))
    assert res.status_code == 200
...
def test_listar_correlatividades_filtra_por_carrera(client, seed, tokens, db):
    ...
    db.add(PensumMateria(carrera_id=seed["carrera"].id, materia_id=avanzada.id, semestre=2, creditos=4))
...
def test_obtener_malla_carrera(client, seed, tokens, db):
    ...
    db.add(PensumMateria(carrera_id=seed["carrera"].id, materia_id=materia.id, semestre=2, creditos=8))
    db.commit()

    res = client.get(f"/pensum/carreras/{seed['carrera'].id}", headers=auth(tokens["alumno"]))
    assert res.status_code == 200
...
def test_avance_alumno_estados(client, seed, tokens, db):
    ...
    db.add(PensumMateria(carrera_id=seed["carrera"].id, materia_id=aprobada.id, semestre=1, creditos=5))
    db.add(PensumMateria(carrera_id=seed["carrera"].id, materia_id=bloqueada.id, semestre=2, creditos=5))
    db.add(Puntaje(user_id=seed["alumno"].id, oferta_materia_id=oferta_aprobada.id, tipo="final", valor=9.0))
    db.add(Correlatividad(materia_id=bloqueada.id, prerrequisito_id=prereq.id, tipo="aprobada"))
    db.commit()

    res = client.get(f"/pensum/alumno/{seed['alumno'].id}/avance", headers=auth(tokens["alumno"]))
    assert res.status_code == 200
...
def test_avance_alumno_no_autorizado_403(client, seed, tokens):
    res = client.get(f"/pensum/alumno/{seed['alumno'].id}/avance", headers=auth(tokens["alumno2"]))
    assert res.status_code == 403


def test_creditos_alumno(client, seed, tokens, db):
    aprobada, oferta_aprobada = _materia_con_oferta(db, seed, "Creditos Aprobada")
    db.add(PensumMateria(carrera_id=seed["carrera"].id, materia_id=aprobada.id, semestre=1, creditos=6))
    db.add(Puntaje(user_id=seed["alumno"].id, oferta_materia_id=oferta_aprobada.id, tipo="final", valor=7.0))
    db.commit()

    res = client.get(f"/pensum/alumno/{seed['alumno'].id}/creditos", headers=auth(tokens["admin"]))
    assert res.status_code == 200
...
def test_inscripcion_bloqueada_por_correlatividad(client, seed, tokens, db):
    ...
    res = client.post("/inscripciones/", json={
        "alumno_id": seed["alumno"].id, "materia_id": avanzada.id,
    }, headers=auth(tokens["alumno"]))
    assert res.status_code == 422
