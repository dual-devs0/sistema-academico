...
def _crear_hilo(db, seed):
    ...
    if not existente:
        db.add(
            Inscripcion(
                alumno_id=seed["alumno"].id, oferta_materia_id=seed["oferta"].id
            )
        )
        db.commit()
...
def test_editar_mensaje_fuera_de_ventana_403(client, seed, tokens, db):
    ...
    res = client.patch(
        f"/foro/mensajes/{msg.id}",
        json={"contenido": "tarde"},
        headers=auth(tokens["alumno"]),
    )
...
def test_editar_mensaje_de_otro_usuario_403(client, seed, tokens, db):
    ...
    res = client.patch(
        f"/foro/mensajes/{mensaje_id}",
        json={"contenido": "hackeado"},
        headers=auth(tokens["alumno2"]),
    )
    assert res.status_code == 403
...
def test_fijar_hilo_profesor_no_titular_403(client, seed, tokens, db):
    ...
    hilo = ForoHilo(
        materia_id=otra_materia.id,
        titulo="Hilo de otra materia",
        creado_por=seed["admin"].id,
    )
    db.add(hilo)
    db.commit()
    db.refresh(hilo)
...
def test_paginacion_mensajes(client, seed, tokens, db):
    ...
    for i in range(5):
        db.add(
            ForoMensaje(
                hilo_id=hilo.id, user_id=seed["alumno"].id, contenido=f"msg {i}"
            )
        )
    db.commit()

    res = client.get(
        f"/foro/hilos/{hilo.id}/mensajes?skip=0&limit=2", headers=auth(tokens["alumno"])
    )
    assert res.status_code == 200
    data = res.json()
    assert data["total"] == 5
