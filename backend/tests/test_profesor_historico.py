...
def test_historico_agrupa_por_periodo(client, seed, tokens, db):
    ...
    # parcial1=5.0 sobre max 20 (pesos default) -> 5/20*10 = 2.5 reescalado a
    # /10, igual que calcula boleta/alumno para esa misma nota -- no el AVG
    # crudo (5.0) que usaba la version vieja de este endpoint.
    assert cat_2025["promedio_grupo"] == 2.5
