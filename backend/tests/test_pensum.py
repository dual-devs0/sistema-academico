...
def _crear_materia_con_oferta(db, seed, nombre, profesor_id=None):
    ...
    oferta = OfertaMateria(
        materia_id=m.id,
        profesor_id=profesor_id or seed["profesor"].id,
        periodo="2026-1",
        activa=True,
    )
    db.add(oferta)
...
def test_prerrequisito_cursando_con_inscripcion(seed, db):
    ...
    db.add(
        Correlatividad(
            materia_id=avanzada.id, prerrequisito_id=base.id, tipo="cursando"
        )
    )
...
def test_multiples_prerrequisitos_acumula_todos_los_pendientes(seed, db):
    ...
    db.add(
        Correlatividad(
            materia_id=avanzada.id, prerrequisito_id=base1.id, tipo="aprobada"
        )
    )
    db.add(
        Correlatividad(
            materia_id=avanzada.id, prerrequisito_id=base2.id, tipo="cursando"
        )
    )
    db.commit()
