...
def test_agenda_vacia_sin_eventos(client, seed, tokens):
    ...
    res = client.get(
        f"/profesor/mi-agenda?desde={desde}&hasta={hasta}",
...
def test_agenda_incluye_clase_fija(client, seed, tokens, db):
    ...
    res = client.get(
        f"/profesor/mi-agenda?desde={hoy.isoformat()}&hasta={hoy.isoformat()}",
        headers=auth(tokens["profesor"]),
    )
    assert res.status_code == 200
    items = res.json()["items"]
    assert any(
        i["tipo"] == "clase" and i["materia_id"] == seed["materia"].id for i in items
    )
...
def test_crear_editar_eliminar_recordatorio(client, seed, tokens):
    ...
    res = client.post(
        "/profesor/recordatorios", json=payload, headers=auth(tokens["profesor"])
    )
    assert res.status_code == 200
    rec_id = res.json()["id"]
    ...
    res = client.patch(
        f"/profesor/recordatorios/{rec_id}",
    ...
    res = client.delete(
        f"/profesor/recordatorios/{rec_id}", headers=auth(tokens["profesor"])
    )
    assert res.status_code == 200
...
def test_recordatorio_de_otro_profesor_403(client, seed, tokens, db):
    ...
    token_otro = create_access_token(
        {"sub": otro.username, "role": "profesor", "user_id": otro.id}
    )

    res = client.post(
        "/profesor/recordatorios",
        json={"titulo": "Propio", "fecha": datetime.now().isoformat()},
