...
| Fase 9 — Auditoría mobile + integración backend completa | COMPLETA | 2026-07-16 | 2026-07-16 | Auditoría integral de la app mobile: TypeScript 0 errores ✅, tests 10/10 ✅. Backend verificado operativo. Endpoints creados: `GET /notas/materia/{id}/detalle` y `GET /notas/materia/{id}/asistencia` en `notas_router.py` — los dos únicos endpoints faltantes para cobertura completa de la app mobile. `/examenes/*` ya existía (Fase 7E). Conexión app→backend configurada a `http://192.168.100.14:8000`. CHANGELOG_MOBILE.md y ESTADO_FASES.md actualizados. Pendiente: reiniciar servidor backend y probar login real con datos seed. |
